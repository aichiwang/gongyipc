...
// svn 设置的备份文件夹
var svnPath = require('./svnConfig').svn;
//var backupPath = require('./svnConfig').backupPath;
var pubPath = svnPath.replace(/\//ig, '\\');
...
// 判断备份文件夹是都存在
var folder_exists = fs.existsSync('E:/SVN/'+fileInfo);
//console.log(fs.existsSync('E:/SVN/backup//')); // ----> true
if(folder_exists) {
...
function sendBackup(){
	exec('DEL '+pubPath+ execPath+ ' /S/Q', function (error, stdout, stderr, stdin) {
		if (error !== null) {
		...
		exec('xcopy '+ filePath + '\\dist '+pubPath+execPath + ' /e /y', function (error, stdout, stderr, stdin) {
			if (error !== null) {
				console.log('exec error: ' + error);
			}
			//console.log(stdout);
